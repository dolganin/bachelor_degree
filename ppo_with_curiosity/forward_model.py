...
class ForwardModelCNN(nn.Module):
    ...
    def __init__(self, action_dim: int, image_channels: int = 1, input_height: int = 120, 
                 input_width: int = 130, hidden_dim: int = 512):
        ...
        self.decoder = nn.Sequential(
            ...
            nn.ConvTranspose2d(hidden_dim, 256, kernel_size=4, stride=2, padding=1),  # (256, ?H, ?W)
            nn.ReLU(),
            nn.BatchNorm2d(256),
            ...
            nn.ConvTranspose2d(128, 64, kernel_size=4, stride=2, padding=1),  # (64, ?H, ?W)
            nn.ReLU(),
            nn.BatchNorm2d(64),
            
            # Четвёртый транспонированный свёрточный слой
            nn.ConvTranspose2d(64, 32, kernel_size=4, stride=2, padding=1),  # (32, ?H, ?W)
            nn.ReLU(),
            nn.BatchNorm2d(32),
            
            # Пятый транспонированный свёрточный слой
            nn.ConvTranspose2d(32, image_channels, kernel_size=4, stride=2, padding=1),  # (image_channels, ?H, ?W)
            nn.Sigmoid()  # Нормализуем выходные значения в диапазон [0, 1]
        )
        
        # Дополнительный слой Conv2d для корректировки размеров изображения
        self.final_conv = nn.Conv2d(image_channels, image_channels, kernel_size=3, padding=1)
    ...
    def forward(self, state: torch.Tensor, action: torch.Tensor) -> torch.Tensor:
        ...
        decoded = self.decoder(combined)  # (batch_size, image_channels, ?, ?)
        
        # Корректировка размера до входных размеров с помощью финальной свертки
        predicted_next_state = nn.functional.interpolate(
            decoded, size=(self.input_height, self.input_width), mode='bilinear', align_corners=False
        )
        predicted_next_state = self.final_conv(predicted_next_state)  # (batch_size, image_channels, input_height, input_width)
